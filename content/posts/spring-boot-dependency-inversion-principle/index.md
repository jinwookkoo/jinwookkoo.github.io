...
`Person`(고수준 모듈)이 `Car`(저수준 모듈)에 직접 의존하고 있다. 만약 자동차가 아닌 지하철이나 버스로 이동해야 하는 상황이 오면, `Person`의 필드 타입을 `Subway`나 `Bus`로 바꿔야 하고 `move` 메소드도 함께 수정해야 한다. 즉, 구체적으로 구현된 저수준 모듈에 의존하다 보니 클래스 간의 결합도가 높아지고, 그만큼 유지보수가 어려워진다.
